...
func GenerateManPage(cmd *cobra.Command, releaseDate string) string {
	...
	// CONFIGURATION
	buf.WriteString(".SH CONFIGURATION\n")
	buf.WriteString("Diffmantic reads its user configuration from\n")
	buf.WriteString("\\fB~/.config/diffmantic/config.yaml\\fR (or \\fBconfig.yml\\fR) on Unix,\n")
	buf.WriteString("or \\fB%APPDATA%\\ediffmantic\\econfig.yaml\\fR on Windows.\n")
	buf.WriteString(".PP\n")
	buf.WriteString("Available settings include:\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B format\n")
	buf.WriteString(".br\n")
	buf.WriteString("Default diff output format (\\fIside-by-side\\fR, \\fIinline\\fR, \\fIjson\\fR,\n")
	buf.WriteString("\\fIactions\\fR).\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B tab_width\n")
	buf.WriteString(".br\n")
	buf.WriteString("Number of spaces per tab stop (default: 4).\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B ignore_comments\n")
	buf.WriteString(".br\n")
	buf.WriteString("Boolean (\\fItrue\\fR or \\fIfalse\\fR) to ignore comments during AST diffing.\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B parse_error_limit\n")
	buf.WriteString(".br\n")
	buf.WriteString("Maximum parse errors permitted before falling back to line diffing.\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B size_limit\n")
	buf.WriteString(".br\n")
	buf.WriteString("Maximum file size in KB for AST parsing.\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".B line_limit\n")
	buf.WriteString(".br\n")
	buf.WriteString("Maximum line count for AST parsing.\n")
	...
	buf.WriteString(".TP\n")
	buf.WriteString(".I ~/.config/diffmantic/config.yaml\n")
	buf.WriteString(".br\n")
	buf.WriteString("User configuration file on Linux and macOS (also accepts config.yml).\n")
	buf.WriteString(".TP\n")
	buf.WriteString(".I %APPDATA%\\ediffmantic\\econfig.yaml\n")
	buf.WriteString(".br\n")
	buf.WriteString("User configuration file on Windows (also accepts config.yml or %LOCALAPPDATA%).\n")
